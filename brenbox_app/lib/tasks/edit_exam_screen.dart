...
class EditExamScreen extends StatefulWidget {
  ...
  const EditExamScreen({Key? key, required this.examData}) : super(key: key);
...
class _EditExamScreenState extends State<EditExamScreen> {
  ...
  void initState() {
    ...
    print('📝 Edit exam screen received data: ${widget.examData}');

    _examNameController = TextEditingController(
      text: widget.examData['examName'] ?? '',
    );
    _venueController = TextEditingController(
      text: widget.examData['venue'] ?? '',
    );

    _selectedSubject = widget.examData['subject']?.isNotEmpty == true
        ? widget.examData['subject']
        : null;
    _selectedType = widget.examData['type'];
    _selectedMode = widget.examData['mode'];

    final examDateValue = widget.examData['examDate'];
    if (examDateValue is Timestamp) {
      _examDate = examDateValue.toDate();
    } else if (examDateValue is DateTime) {
      _examDate = examDateValue;
    } else {
      _examDate = DateTime.now();
    }

    final startTimeValue = widget.examData['startTime'];
    DateTime startDateTime;
    if (startTimeValue is Timestamp) {
      startDateTime = startTimeValue.toDate();
    } else if (startTimeValue is DateTime) {
    ...
    final endTimeValue = widget.examData['endTime'];
    DateTime endDateTime;
    if (endTimeValue is Timestamp) {
      endDateTime = endTimeValue.toDate();
    } else if (endTimeValue is DateTime) {
        ...
        if (endMins <= startMins) {
          ...
          Future.delayed(const Duration(milliseconds: 100), () {
            if (mounted) {
              ...
              _showError(
                'Time Validation',
                'End time has been adjusted to be after $h:$m $p',
              );
            }
          });
        }
  ...
  Future<Map<String, dynamic>?> _checkExamClash(
    ...
    for (var doc in existingExams.docs) {
      try {
        if (doc.id == widget.examData['id']) continue;
      ...
      await showDialog(
        ...
        builder: (dialogContext) {
          return AlertDialog(
            ...
            content: Text('Exam has been successfully updated',
  ...
  Widget _buildTypeSelector() {
    return Row(
      children: _examTypes.map((type) {
        ...
        return Expanded(
          child: Padding(
            padding: EdgeInsets.only(right: type != _examTypes.last ? 8 : 0),
            child: GestureDetector(
              onTap: () => setState(() => _selectedType = type),
              child: Container(
                padding: const EdgeInsets.symmetric(vertical: 12),
                decoration: BoxDecoration(
                  color: isSelected ? const Color(0xFFFEFFE6) : AppColors.card(context),
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color:
                        isSelected ? const Color(0xFF9AB900) : AppColors.border(context),
                    width: 2,
                  ),
                ),
                child: Text(
                  ...
                  style: GoogleFonts.dmMono(
                      fontSize: 13,
                      fontWeight: FontWeight.bold,
                      color: isSelected ? const Color(0xFF9AB900) : AppColors.text(context)),
                ),
              ),
            ),
          ),
  ...
  Widget _buildModeSelector() {
    return Row(
      children: _examModes.map((mode) {
        ...
        return Expanded(
          child: Padding(
            padding: EdgeInsets.only(right: mode != _examModes.last ? 8 : 0),
            child: GestureDetector(
        ...
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: ColorScheme.light(
              primary: const Color(0xFF9AB900),
              onPrimary: Colors.white,
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.access_time,
                        size: 20, color: Color(0xFF9AB900)),
                      ...
                      child: Row(
                        ...
                        children: [
                          const Icon(Icons.touch_app,
                              size: 12, color: Color(0xFF9AB900)),
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(
                ...
                borderSide: const BorderSide(
                    color: Color(0xFF9AB900), width: 2.5),
              ),
              contentPadding:
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        height: 40,
        decoration: BoxDecoration(
          color: selected ? const Color(0xFF9AB900) : AppColors.input(context),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
            color: selected ? const Color(0xFF9AB900) : AppColors.border(context),
            width: 2,
          ),
        ),
